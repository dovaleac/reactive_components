 ...
 *
 * <p>Copyright 2019 Pablo González Doval
 *
...
import com.dovaleac.flowables.composition.strategy.instance.buffered.buffer.read.ReadBufferImpl;
import com.dovaleac.flowables.composition.strategy.instance.buffered.buffer.write.WriteBuffer;
import com.dovaleac.flowables.composition.strategy.instance.buffered.buffer.write.WriteBufferAcceptNewInputsTrigger;
...
public class UnmatchedYetRemnantImpl<T, OT, KT, LT, RT>
  ...
  public UnmatchedYetRemnantImpl(
    ...
    this.readBuffer = new ReadBufferImpl<>(function, maxBlocksForReadBuffer, side);
    writeBuffer =
        new WriteBuffer<>(
            this, config.getMaxElementsInWriteBuffer(), config.getInitialMapForWriteBuffer(), side);
  }
  ...
  public Completable addToReadBuffer(List<OT> otherTypeElements) {
    CompletableFuture.runAsync(() -> stateMachine.fire(UnmatchedYetRemnantTrigger.PROCESS_READ));
  ...
  protected Completable pollRead() {
  ...
  protected Completable pollRead(int acc) {
    return Completable.fromAction(
        () -> {
          EventManagerContext.getInstance()
              .getEventManager()
              .processEvent(Event.pollRead(side, acc));
          readBuffer
              .pull()
              ...
              .subscribe(
                  ownTypeElements -> {
                    if (ownTypeElements.isEmpty()) {
                      if (checkCapacity() == NextAction.READ) {
                        Completable.fromAction(() -> Thread.sleep(100))
                            .andThen(pollRead(0))
                            .subscribe();
                      } else {
                          ...
                          .subscribe(
                              () -> {
                                ...
                                if (acc < config.getPollReadsForCheckCapacity()) {
                                  pollRead(acc + 1).subscribe();
                                  return;
                                }
                                if (checkCapacity() == NextAction.READ) {
                                  pollRead(0).subscribe();
                                } else {
                                  stateMachine.fire(
  ...
  protected void consumeWriteBuffer() {
    ...
    writeBuffer.clear();
    writeBuffer.fire(WriteBufferAcceptNewInputsTrigger.UNFREEZE);
  ...
  private NextAction checkCapacity() {
    return config
        ...
        .getNextActionWithLog(readBuffer.getCapacity(), writeBuffer.getCapacity(), side);
  }
  ...
  protected boolean tryToEmit(KT key, OT ot, Map<KT, T> map) {
    ...
    if (oneT == null) {
      ...
      EventManagerContext.getInstance()
          .getEventManager()
          .processEvent(Event.coupleEmitted(side, key.toString()));
      emitInnerJoin(ot, map.remove(key));
      return false;
    }
  }
  ...
  public Completable addToWriteBuffer(Map<KT, T> ownTypeElements) {
    EventManager eventManager = EventManagerContext.getInstance().getEventManager();
    return Completable.create(
        completableEmitter -> {
          try {
            writeBuffer.addToQueue(ownTypeElements);
            eventManager.processEvent(Event.pushToWriteBuffer(side, ownTypeElements));
            completableEmitter.onComplete();
          } catch (WriteBufferFrozenException ex) {
            eventManager.processEvent(Event.writeBufferFrozen(side));
            completableEmitter.onError(ex);
          } catch (WriteBufferFullException ex) {
            eventManager.processEvent(Event.writeBufferFull(side));
  ...
  void enableConsumingReadingBuffer() {
    CompletableFuture.runAsync(() -> guarder.retakeReading());
    pollRead().subscribe();
  }
  ...
  void syncFinished() {
    ...
    CompletableFuture.runAsync(() -> stateMachine.fire(UnmatchedYetRemnantTrigger.SYNC_FINISHED));
  }
  ...
  public void setSynchronizee() {
    stateMachine.fire(UnmatchedYetRemnantTrigger.CONFLICT_SOLVED_AS_SYNCHRONIZEE);
  }

  public void claimToBeTheSynchronizer() {
