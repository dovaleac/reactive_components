...
public interface LeverageBufferCapacitiesStrategy {
  ...
  default NextAction getNextActionWithLog(
      double readBufferCapacity, double writeBufferCapacity, Side side) {
    NextAction nextAction = getNextAction(readBufferCapacity, writeBufferCapacity);

    EventManagerContext.getInstance()
        .getEventManager()
        .processEvent(
            Event.chooseNextAction(side, nextAction, readBufferCapacity, writeBufferCapacity));
