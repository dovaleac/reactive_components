...
public class LeverageBufferCapacitiesStrategyImpl implements LeverageBufferCapacitiesStrategy {
  ...
  /**
   * Decides whether to keep reading or start writing. All values in range [0.0, 1.0]
   * @param writeThresholdToChange If the write buffer is more full than this %, go to write mode
   * @param readThresholdToChange Else if the read buffer is less full than this %, keep reading
   * @param minWriteToAcceptChange Else if the write buffer is more full than this %, go to write
   *     mode, else keep reading
   */
  public LeverageBufferCapacitiesStrategyImpl(
      double writeThresholdToChange, double readThresholdToChange, double minWriteToAcceptChange) {
